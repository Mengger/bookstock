...
import com.bookrecovery.until.redis.clients.jedis.BinaryClient.LIST_POSITION;
import com.bookrecovery.until.redis.clients.util.Hashing;
import com.bookrecovery.until.redis.clients.util.Sharded;



public class BinaryShardedJedis extends Sharded<Jedis, JedisShardInfo>
	implements BinaryJedisCommands {
	private transient static Log log = LogFactory.getLog(BinaryShardedJedis.class);
	...
	if(log.isDebugEnabled())
		log.error("���ε��������ȡ�����ӵ�ַΪ��"+j.getClient().getHost()+":"+j.getClient().getPort());
	return j.get(key);
    }
