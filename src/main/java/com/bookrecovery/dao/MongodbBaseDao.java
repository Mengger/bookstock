...
import com.bookrecovery.entry.MongoDBPagination;
...
public abstract class MongodbBaseDao<T> {
	 ...
	 * @return
	 */
	public MongoDBPagination<T> getPage(int pageNo, int pageSize, Query query) {
		long totalCount = this.mongoTemplate.count(query, this.getEntityClass());
		MongoDBPagination<T> page = new MongoDBPagination<T>(pageNo, pageSize, totalCount);
	...
	public void findAllAndRemove(Query query) {
        this.mongoTemplate.findAllAndRemove(query, this.getEntityClass());
    }
