...
public class FtpUtil {
	public static final String FILE_SPEARATROR = "/";// add by lxm 08-10-27
														// 文件分隔符,只适用于unix环境
	...
	public FtpUtil() throws Exception {
		FTPBean ftpBean = FTPFactory.FTPBean;
		client = new FTPClient();
		client.connect(ftpBean.getHostIp(), ftpBean.getPort());
		client.login(ftpBean.getUserName(), ftpBean.getPassword());
		boolean changeResult=client.changeWorkingDirectory(ftpBean.getRemotePath());
		if(!changeResult) throw new Exception("change working directory fail");
		localPath = ftpBean.getLocalPath();
		remotePathHis = ftpBean.getRemotePathHis();// add by lxm 08-10-27
		// add by huangyb
		_host = ftpBean.getHostIp();
		_port = ftpBean.getPort();
		_username = ftpBean.getUserName();
		_password = ftpBean.getPassword();
		_remotePath = ftpBean.getRemotePath();
		_localPath = ftpBean.getLocalPath();
		_localPathTemp = ftpBean.getLocalPathTemp();
	 ...
	 * @throws Exception
	 */
	public boolean upload(String remoteFileName, InputStream input, int mode) throws Exception {
		if (mode == BIN) {
			client.setFileType(FTP.BINARY_FILE_TYPE);
		} else if (mode == ASC) {
			client.setFileType(FTP.ASCII_FILE_TYPE);
		...
		return upload(remoteFileName, input);
	}
	 ...
	 * @throws Exception
	 */
	public boolean upload(String remoteFileName, InputStream input) throws Exception {
		return client.storeFile(remoteFileName, input);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		f.bin();
		System.out.println(f.makeDir("sdsghfgsdf"));
		boolean con=f.upload("audio",new FileInputStream(new File("D:\\111.txt")),ASC);
		System.out.println(con);
		/*
		 * String str2 = new String("电视剧"); f.client.makeDirectory(new
		 * String(str2.getBytes("GBK"),"iso-8859-1"));
		 */

		/*FileOutputStream out = new FileOutputStream("d:\\aaaa.txt");
		f.download("aa", out, ASC);*/

		//f.download("aa",out,BIN);
		...
		/*
		 * for (int i = 0; i < 1; i++) { InputStream is = f.readRemote("b"); int
		 * b = 0; while ( (b = is.read()) != -1) { //System.out.println(b); }
		 * f.completePendingCommand(); }
		 */
		// f.download("b", "b");
		// f.delete("b");
	...
	public boolean makeDir(String path) throws Exception {
		if (client != null) {
			...
			for (int i = 0; i < paths.length; i++) {
				String ftpDir = client.printWorkingDirectory();// 获取ftp当前工作目录
				String curDir = ftpDir + FILE_SPEARATROR + paths[i];
				return client.makeDirectory(new String(paths[i].getBytes("UTF-8"), "iso-8859-1"));// 创建指定目录
			}
		}
		return false;
	}
